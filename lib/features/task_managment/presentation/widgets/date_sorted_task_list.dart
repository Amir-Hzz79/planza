...
class _DateSortedTaskListState extends State<DateSortedTaskList> {
  ...
  Widget build(BuildContext context) {
    ...
    List<TaskModel> tasks = widget.taskState is TasksLoadedState
        ? (widget.taskState as TasksLoadedState).tasks
        : List.generate(
            8,
            (index) => TaskModel(
              id: index,
    ...
    tasks.sort(
      (a, b) => widget.sortingOrder == SortOrdering.ascending
          ? (a.dueDate?.isAfter(b.dueDate ?? DateTime(2000)) ?? false ? 1 : -1)
    ...
    final List<Widget> children = [
      if (noDueDateTasks.isNotEmpty)
        TweenAnimationBuilder<double>(
          ...
          tween:
              Tween<double>(begin: 0, end: noDueDateTasks.isNotEmpty ? 1 : 0),
      ...
      if (todayTasks.isNotEmpty)
        TweenAnimationBuilder<double>(
          duration: Duration(milliseconds: 500),
          tween: Tween<double>(begin: 0, end: todayTasks.isNotEmpty ? 1 : 0),
          builder: (context, value, child) {
            return Opacity(opacity: value, child: child);
          },
          child: ExpansionTile(
      ...
      if (futureTasks.isNotEmpty)
        TweenAnimationBuilder<double>(
          duration: Duration(milliseconds: 500),
          tween: Tween<double>(begin: 0, end: futureTasks.isNotEmpty ? 1 : 0),
          builder: (context, value, child) {
            return Opacity(opacity: value, child: child);
          },
          child: ExpansionTile(
    ...
    return Skeletonizer(
      enabled: widget.taskState is! TasksLoadedState,
